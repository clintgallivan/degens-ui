...
export default function TopTokenSection({ props }: any) {
    ...
    const [categoryQueries, setCategoryQueries] = useState(category ? [category] : []);
    ...
    const handleFilterLoad = async () => {
        try {
            const res = await clientApi.get('/api/token-filters', {});
    ...
    const CardHeader = () => {
        return (
            <div className={styles.header_row}>
                ...
                <div onClick={() => setIsExpanded(!isExpanded)} className={styles.filter_btn}>
                    <RetroButton>
                        {isExpanded ? (
    ...
    useEffect(() => {
        const fetchDataTable = async (marketCapMin?: number, marketCapMax?: number) => {
            setQueryIsLoading(true);
            try {
                const res: any = await clientApi.get('/api/leaderboard-query', {
                    params: {
